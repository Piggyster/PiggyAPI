package me.piggyster.api.color;

import com.google.common.collect.ImmutableMap;
import me.piggyster.api.color.pattern.*;
...
public class ColorAPI {
     ...
     * @since 1.0.2
     */
    private static final List<Pattern> PATTERNS = Arrays.asList(new GradientPattern(), new SolidPattern(), new RainbowPattern(), new AltSolidPattern(), new RawSolidPattern());
     ...
     */
    @Nonnull
    public static String gradient(@Nonnull String string, @Nonnull Color start, @Nonnull Color end) {
    ...
    @Nonnull
    private static ChatColor[] createGlow(@Nonnull Color start, @Nonnull Color end, int step) {
        //step = (int) Math.ceil(step / 2f);
        ChatColor[] colors = new ChatColor[step];
        int stepR = Math.abs(start.getRed() - end.getRed()) / (step - 1);
        int stepG = Math.abs(start.getGreen() - end.getGreen()) / (step - 1);
        int stepB = Math.abs(start.getBlue() - end.getBlue()) / (step - 1);
